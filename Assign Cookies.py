...
class Solution(object):
    def findContentChildren(self, g, s):
        ...
        j=0
        output = 0
        while i<len(s_tup) and j<len(g_tup):
            if s_tup[i]==g_tup[j]:
                output += min(s.count(s_tup[i]),g.count(s_tup[i]))
            if s_tup[i]> g_tup[j]:
                j += 1
            else:
                i += 1
...
class Solution(object):
    def findContentChildren(self, g, s):
        ...
        g_tup = tuple(set(g))
        s_tup = tuple(set(s))
        nums = tuple(set(g_tup + s_tup))
        for i in nums:
            count_g = g.count(i)
            count_s = s.count(i)
            output += min(count_g, count_s)
        return output
'''
## till now all sol failed ; clearly my approach is wrong -- actually i read the question wrong
